...
typedef void* (*is_local_cb)(const uint8_t* mac, const char* ifname, const uint16_t);
typedef int (*update_lease_cb)(const char* ifname, const uint16_t vlanid, const uint8_t* mac, const struct in_addr* ip, uint32_t* expiresAt);
