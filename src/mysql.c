...
void mysql_update_lease(const uint8_t* mac, const struct in_addr* yip, const char* ifname, const int vlanid, const uint32_t expiresAt, const enum t_lease_update_src reason)
{
	...
	char sql[2048];
	if (expiresAt > now) {
		snprintf(sql, sizeof(sql), "INSERT INTO " MYSQLLEASETABLE " (bridge, mac, ip, validUntil) VALUES('%s', '%s', '%s', %d + UNIX_TIMESTAMP()) ON DUPLICATE KEY UPDATE validUntil = %d + UNIX_TIMESTAMP();", sql_esc_bridge, ether_ntoa_z((struct ether_addr *)mac), inet_ntoa(*yip), expiresAt - now, expiresAt - now);
	} else {
		snprintf(sql, sizeof(sql), "UPDATE " MYSQLLEASETABLE " SET validUntil = 0 WHERE bridge = '%s' AND mac = '%s';", sql_esc_bridge, ether_ntoa_z((struct ether_addr *)mac));
	}
