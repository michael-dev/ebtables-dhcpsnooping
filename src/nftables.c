...
static void nftables_novlan(const char* op, const struct in_addr* yip, const uint8_t* mac, const char* ifname) {
	char cmd[65535];
	...
	if (legacy) {
		...
		snprintf(cmd, sizeof(cmd), NFTABLES " %s element bridge filter " SETNAME " { \"%s\" . %s . %s }",
			 op, ifname, ether_ntoa_z((struct ether_addr *)mac), inet_ntoa(*yip));
		nftables_run(cmd);
		snprintf(cmd, sizeof(cmd), NFTABLES " %s element bridge nat " MAPNAME " { \"%s\" . %s : %s }",
			 op, ifname, inet_ntoa(*yip), ether_ntoa_z((struct ether_addr *)mac));
		nftables_run(cmd);
	}
}

static void nftables_vlan(const char* op, const struct in_addr* yip, const uint8_t* mac, const char* ifname, const uint16_t vlanid) {
	char cmd[65535];
	...
	if (legacy) {
		...
		snprintf(cmd, sizeof(cmd), NFTABLES " %s element bridge filter " SETNAME " { \"%s\" . %d . %s . %s }",
			 op, ifname, (int) vlanid, ether_ntoa_z((struct ether_addr *)mac), inet_ntoa(*yip));
		nftables_run(cmd);
		snprintf(cmd, sizeof(cmd), NFTABLES " %s element bridge nat " MAPNAME " { \"%s\" . %d . %s : %s }",
			 op, ifname, (int) vlanid, inet_ntoa(*yip), ether_ntoa_z((struct ether_addr *)mac));
		nftables_run(cmd);
	}
}

static void nftables_do(const char* ifname, const uint16_t vlanid, const uint8_t* mac, const struct in_addr* ip, const int start)
{
	if (disabled)
		return;
	...
	eprintf(DEBUG_VERBOSE, "%s nftables rule: MAC: %s IP: %s BRIDGE: %s VLAN: %d", op, ether_ntoa_z((struct ether_addr *)mac), inet_ntoa(*yip), ifname, (int) vlanid);
	if (vlanid == 0)
		nftables_novlan(op, yip, mac, ifname);
	else
		nftables_vlan(op, yip, mac, ifname, vlanid);
}
